...
firstDigit :: Int -> Int
firstDigit n = (abs n) `div` 10 ^ floor (logBase 10 (fromIntegral (abs n)))
...
string.
-}
subString start end str = drop (if start < 0 then 0 else start) $ (if end < 0 then "" else take (end + 1) str)
...
-}
strSum str = sum $ map (\x -> read x :: Integer) $ words str
...
lowerAndGreater n list = show n ++ " is greater than " ++ show x ++ " elements and lower than " ++ show y ++ " elements"
    where
        ...
        go _ [] (x, y) = (x, y)
        go n (el:els) (x, y) = go n els (if n > el then x + 1 else x, if n < el then y + 1 else y)
