...
-- duplicate xs = xs >>= (\x -> [x, x]) -- using Bind
...
data WinResult a = WinResult
  { gold :: GoldChest,
    treasure :: Maybe a,
    expeience :: Int
  }

data FightOutcome a = RUN | DIE | WIN (WinResult a)

expGain :: DragonType -> Int
expGain d = case d of
  RedDragon -> 100
  BlackDragon -> 150
  GreenDragon -> 250

getTreasure :: DragonType -> Chest a -> Maybe a
getTreasure d c = case d of
  GreenDragon -> Nothing
  _ -> chestTreasure c

--- Example DATA

data TreasureType = SWORD | CROWN

stolenTreasure :: TreasureType
stolenTreasure = SWORD

stolenChest :: Chest TreasureType
stolenChest =
  Chest
    (Gold 100)
    (Just stolenTreasure)

dragonFight :: Knight -> Dragon -> FightOutcome TreasureType
dragonFight (Knight kh ka ke) (Dragon dt dh df) = fight kh ka ke dh 0
  where
    fight :: Int -> Int -> Int -> Int -> Int -> FightOutcome TreasureType
    fight knightHp attack endurance dragonHp strikes
      | knightHp <= 0 = DIE
      | endurance <= 0 = RUN
      | dragonHp <= 0 = WIN (WinResult (chestGold stolenChest) (getTreasure dt stolenChest) (expGain dt))
      | otherwise =
          let newDragonHp = dragonHp - attack
              newKnightHp = if strikes `mod` 10 == 9 then knightHp - df else knightHp
           in fight newKnightHp attack (endurance - 1) newDragonHp (strikes + 1)
...
eval vars expr = case expr of
  Var a -> maybe (Left (VariableNotFound a)) Right (lookup a vars)
  Add a b -> do
    va <- eval vars a
    vb <- eval vars b
    return (va + vb)
...
constantFolding = rebuild . optimize . flatten
  where
    flatten :: Expr -> [Expr]
    ...
    optimize :: [Expr] -> ([Expr], Int)
    optimize = foldr reducer ([], 0)
      where
        reducer (Lit n) (vars, total) = (vars, total + n)
        reducer v (vars, total) = (v : vars, total)
